...
class GCN(nn.Module):
    def __init__(self, nfeat, nhid, nclass, adj_matrix, keep_prob):
    ...
    def forward(self, x):
        x = F.relu(self.gc1(x, self._adj_matrix))
        x = F.dropout(x, self.dropout, training=self.training)
        x = self.gc2(x, self._adj_matrix)
        # return F.sigmoid(x, dim=1)
        return x


# class fGSL(nn.Module):
#     def __init__(self, input_dim, time_step,  in_matrix,out_matrix,keep_prob):
#         super(fGSL, self).__init__()
